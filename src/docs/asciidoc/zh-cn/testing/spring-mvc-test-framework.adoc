...
`WebTestClient` 还可以连接到实时服务器并执行完整的端到端集成测试.  Spring Boot 也支持此功能， 您可以在其中测试 {docs-spring-boot}/html/spring-boot-features.html#boot-features-testing-spring-boot-applications-testing-with-running-server[测试正在运行的服务器].
